...
public class GestorTransacciones {
    ...
    private DetectorComportamiento  detector = null;
    private final GrafoTransferencias     grafo = new GrafoTransferencias();

    // Spring inyecta los tres beans por constructor ──────────────────────────
    public GestorTransacciones(GestorUsuarios gu,
                               GestorBilleteras gb) {
        this.gu       = gu;
        this.gb       = gb;
    ...
    public boolean recargar(String uid, String bid, double monto) {
        ...
        Billetera b = gb.buscarPorId(bid);
        ...
        b.agregarTransaccion(tx);
        u.registrarTransaccionReversible(tx);
        int pts = u.getPuntosTotales();
        u.sumarPuntos(tx.getPuntosGenerados());
        gu.actualizarArbolPuntos(u, pts);
        detector.analizar(u, b, tx);
    ...
    public boolean retirar(String uid, String bid, double monto) {
        Usuario u = gu.buscarPorId(uid);
        Billetera b = gb.buscarPorId(bid);
        if (u == null || b == null || !b.isActiva()) {
            System.out.println("❌ Datos inválidos."); return false;
        }
        if (!b.retirar(monto)) {
            System.out.println("❌ Saldo insuficiente: $" + b.getSaldo());
            u.agregarNotificacion("Retiro rechazado en " + b.getNombre());
            return false;
        }
    ...
    public boolean transferir(String uidOrigen, String bidOrigen,
                              String bidDestino, double monto) {
        Usuario   uO = gu.buscarPorId(uidOrigen);
        Billetera bO = gb.buscarPorId(bidOrigen);
        Billetera bD = gb.buscarPorId(bidDestino);
        ...
        bO.agregarTransaccion(txE);
        bD.agregarTransaccion(txR);
        uO.registrarTransaccionReversible(txE);
        int pts = uO.getPuntosTotales();
        uO.sumarPuntos(txE.getPuntosGenerados());
        gu.actualizarArbolPuntos(uO, pts);
        if (!bD.getUsuarioId().equals(uidOrigen)) {
            Usuario uD = gu.buscarPorId(bD.getUsuarioId());
            if (uD != null) {
                int ptD = uD.getPuntosTotales();
                uD.sumarPuntos(txR.getPuntosGenerados());
    ...
    public boolean revertirUltimaOperacion(String uid) {
        ...
        if (tx.getEstado() == EstadoTransaccion.REVERTIDA) {
            System.out.println("❌ Ya revertida."); return false;
        }
        switch (tx.getTipo()) {
            case RECARGA -> {
        ...
        int pts = u.getPuntosTotales();
        u.descontarPuntos(tx.getPuntosGenerados());
        gu.actualizarArbolPuntos(u, pts);
        tx.revertir();
        u.agregarNotificacion("Operación revertida: " + tx.getId());
        System.out.printf("✅ Operación [%s] revertida. Puntos: %d%n",
                tx.getId(), u.getPuntosTotales());
        return true;
    }
