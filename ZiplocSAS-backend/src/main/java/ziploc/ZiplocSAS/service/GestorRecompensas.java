...
public class GestorRecompensas {
    ...
    public GestorRecompensas(GestorUsuarios gu) {
        this.gu = gu; inicializarCatalogo();
    }
    ...
    public boolean canjearBeneficio(String uid, String bid) {
        ...
        Beneficio b = null;
        for (Beneficio ben : catalogo) if (ben.getId().equals(bid)) { b = ben; break; }
        if (b == null) { System.out.println("❌ Beneficio no encontrado."); return false; }
        if (u.getPuntosTotales() < b.getPuntosRequeridos()) {
            System.out.println("❌ Puntos insuficientes. Tienes " + u.getPuntosTotales()); return false;
        }
        int pts = u.getPuntosTotales(); u.descontarPuntos(b.getPuntosRequeridos());
        gu.actualizarArbolPuntos(u, pts);
        if (b.getTipo() == Beneficio.TipoBeneficio.BONO_PUNTOS) {
            int p = u.getPuntosTotales(); u.sumarPuntos(50); gu.actualizarArbolPuntos(u, p);
        }
        u.agregarNotificacion("🎁 Canjeaste: " + b.getDescripcion());
        System.out.println("✅ Beneficio canjeado: " + b); return true;
    }
