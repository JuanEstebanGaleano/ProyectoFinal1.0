...
public class DetectorComportamiento {
    ...
    public void analizar(Usuario usuario, Billetera billetera, Transaccion tx) {
        ...
        boolean sospechosa = false; NivelRiesgo riesgo = NivelRiesgo.BAJO;

        long txRapidas = rec.stream().filter(t -> t.getTipo() == TipoTransaccion.TRANSFERENCIA_ENVIADA).count();
        if (txRapidas >= LIMITE_TX) {
            sospechosa = true; riesgo = NivelRiesgo.MEDIO;
            usuario.agregarNotificacion("🚨 " + txRapidas + " transferencias en " + VENTANA_MIN + " min");
        }

        double prom = calcularPromedio(usuario);
        if (prom > 0 && tx.getValor() > prom * FACTOR) {
            sospechosa = true; riesgo = NivelRiesgo.ALTO;
            usuario.agregarNotificacion(String.format("🚨 Monto inusual: $%.2f (prom $%.2f)", tx.getValor(), prom));
        }

        if (tx.getTipo() == TipoTransaccion.RETIRO && billetera.getSaldo() < 10 && tx.getValor() > 500) {
            sospechosa = true; riesgo = NivelRiesgo.ALTO;
        ...
        if (sospechosa) {
            tx.setSospechosa(true); tx.setNivelRiesgo(riesgo);
